        ...
        public async Task<IActionResult> OnPostAsync()
        {
            if (ModelState.IsValid)
            {
                ...
                await _emailSender.SendEmailAsync(
                    Input.Email,
                    IdentityResources.ResetPassword,
                    $"{IdentityResources.PleaseResetYourPasswordBy} <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>{IdentityResources.ClickingHere}</a>.");
